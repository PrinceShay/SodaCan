...
export default function Preloader({ onFinish }: PreloaderProps) {
  const loader = useRef(null); // Referenz für den Ladebalken
  const loaderContainer = useRef(null); // Referenz für den Ladecontainer

  const { progress } = useProgress(); // Fortschrittswert des Ladeprozesses
  const percent = Math.round(progress * 100) / 100;

  useGLTF.preload("/can.glb"); // Vorladen des GLTF-Modells

  // Funktion, die ausgeführt wird, wenn der Preloader abgeschlossen ist
  const loaderFinished = () => {
    if (onFinish) {
      onFinish(); // Informiert die übergeordnete Komponente (z. B. Hero), dass das Laden abgeschlossen ist
    }
  };

  // Verwenden von GSAP, um Animationen zu erstellen
  useGSAP(
    () => {
      const loadingTimeline = gsap.timeline();
      loadingTimeline.to(loader.current, {
        height: `${progress}%`, // Dynamische Anpassung der Höhe basierend auf dem Fortschritt
        ease: "power1.out",
        duration: 0.2,
      });

      if (progress === 100) {
        const loaderOutTimeline = gsap.timeline();
        loaderOutTimeline
          .to(loaderContainer.current, {
            ...
            duration: 1,
            ease: "power2.out",
            onComplete: loaderFinished, // Ruft die Funktion auf, wenn die Animation fertig ist
          })
          .set(loaderContainer.current, { display: "none" }); // Entfernt den Container aus dem DOM-Flow
      }
    },
    { scope: loaderContainer, dependencies: [progress, loaderFinished] },
  );

  return (
    <div
      ref={loaderContainer}
      className="w-full h-screen bg-[#0C1110] fixed z-[100] flex flex-col items-center justify-center"
    >
      <div className="flex flex-col items-center justify-center">
        <div className="relative">
          <div className="relative flex flex-col items-center justify-center gap-6">
            ...
            <div
              ref={loader}
              className="w-32 absolute overflow-hidden top-0 h-0"
            ...
            <p className="font-bebasNeue text-4xl text-center leading-none">
